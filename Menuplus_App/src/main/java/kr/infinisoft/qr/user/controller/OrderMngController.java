	...
	public String loginPage(HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		try {
			...
			if(appToken.getValue() != null) {
				logger.info("login decodeAppToken : " + appToken.getValue());
			}
...
//			logger.info("email : " + findData.get("email"));
	...
	public JSONObject changeStatus(@RequestBody Map<String, Object> changeData, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String waitTime = String.valueOf(changeData.get("waitTime"));
			
			CloseableHttpClient httpClient = HttpClients.createSystem();
			String body = "";
			int httpStatusCode = -1;
			
			HttpPost httpPost = new HttpPost("https://admin.menuplus.kr/api/app/orderStatus/" + orderCd + "/" + orderStatusCd + "/" + waitTime); //운영
			//HttpPost httpPost = new HttpPost("http://10.10.10.14:8185/api/app/orderStatus/" + orderCd + "/" + orderStatusCd + "/" + waitTime); //운영
			
			HttpResponse httpResponse = httpClient.execute(httpPost);
			httpStatusCode = httpResponse.getStatusLine().getStatusCode();
			
			if(httpStatusCode != 200) {
				throw new Exception("changeStatus Error httpStatus : " + httpStatusCode);
			} else {
				body = EntityUtils.toString(httpResponse.getEntity(), "UTF-8");
	...
	public JSONObject cancelOrder(@RequestBody Map<String, Object> cancelData, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//HttpGet httpGet = new HttpGet("http://10.10.10.14:8185/api/app/cancelOrder/" + orderCd + "/" + orderId + "/" + cancelPw + "/" + cancelReason); //운영
	...
	public Cookie addCookie(HttpServletRequest request, HttpServletResponse response, String cookieKey, String cookieValue) throws Exception{
		...
		try {
			...
			//쿠키 생성
			if(cookieValue != null) {
				cookieData = new Cookie(cookieKey, URLDecoder.decode(cookieValue, "utf-8"));			
			} else {
				cookieData = new Cookie(cookieKey, cookieValue);				
			}
