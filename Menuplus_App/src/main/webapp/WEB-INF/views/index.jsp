...
<head>
	<title>메뉴플러스 주문알림</title>
   	...
   	<jsp:include page="./webSocket.jsp" flush="true"/>
   	<script>
   		$(document).ready(function(){
   	  	...
   	  	function changeOrder(orderCd, orderStatusCd, e) {
   	  		var changeData = new Object();
   	  		changeData.orderCd = orderCd;
   	  		changeData.orderStatusCd = orderStatusCd;
   	  		if(e != "0") {
	   	  		changeData.waitTime = e.parents(".popup").find("#waitTime-" + orderCd).val();   	  			
   	  		} else {
   	  			changeData.waitTime = 0;
   	  		}
...
<%-- 			                			<a class="btn-w" href="#" onclick="changeOrder(${orderList.orderCd}, 30);">주문접수</a> --%>
											<a class="btn-w popup-trigger" popup-class=".popup-time-required-${orderList.orderCd}" href="#">주문접수</a>
			                		</div>
			                	</div>
			                	<div class="popup-wrap popup-time-required-${orderList.orderCd}">
									<div class="popup">
										<div class="popup-con">
											<h3 class="notice-text">예상 소요 시간을 선택해 주세요.</h3>
											<div class="select-wrap">
												<select id="waitTime-${orderList.orderCd}">
										...
										<div class="btn-wrap">
											<a href="#" class="btn btn-l btn-wm btn-gray popup-close">취소</a>
											<a href="#" class="btn btn-l btn-wm btn-d-gray popup-close" 
											onclick="changeOrder(${orderList.orderCd}, 30, $(this));">주문접수</a>
										</div>
									</div>
								</div>
	            			</c:if>
...
<%-- 											</c:when> --%>
					                ...
					                <ul class="goods-li">
						       		...
						       		<div class="btns">
			                			<a class="btn-gray" href="<%= request.getContextPath() %>/app/orderDetail/${orderList.orderCd}">상세보기</a>
			                			<a class="btn-w" href="#" onclick="changeOrder(${orderList.orderCd}, 32, 0);">완료처리</a>
			                		</div>
					        	</div>
					        </c:if>
	               		</c:forEach>
...
<!-- 	               	</li> -->
	    ...
	    <div class="popup-wrap popup-logout">
      	...
      	<div class="toast-popup" onclick="window.location.replace('<%= request.getContextPath() %>/app/index');">
      		새 주문이 들어 왔습니다.
      	</div>
   	</div>
</body>
